...
Understand the regulatory context and importance of TLFs in clinical study reports.
Learn basic concepts of Polars and rtflite for TLF generation.
:::
...
The creation of a CSR is a collaborative effort that involves
...
in RTF format. While Polars handles the data processing and statistical calculations, rtflite focuses exclusively on the presentation layer. The package is designed to:
...
dataset `data/adae.parquet`. This dataset contains adverse event (AE) information
...
The input of `col_rel_width` is a list with the same length as the number of columns.
...
We also allow column headers to be displayed in multiple lines.
If an empty column name is needed for a column,
you can insert an empty string. For example, `["name 1", "", "name 3"]`.

In `RTFColumnHeader`, the `col_rel_width` can be used to align column headers
with different numbers of columns.

By using `RTFColumnHeader` with `col_rel_width`, one can customize complex column headers.
