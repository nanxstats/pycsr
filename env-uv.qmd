...
- Build backends (like `uv_build`) create distributable packages.
...
By default, `uv init` creates a packaged project with a `src/` layout and
configures `uv_build` as its build backend. For a project that only needs an
environment and standalone scripts, use `uv init --no-package`.
...
The generated `uv_build` version range requirement is based on the installed
uv version, so keep the range written by `uv init`.
...
::: callout-important
Use the full `MAJOR.MINOR.PATCH` version (for example, `3.14.0`) rather than
just `MAJOR.MINOR` (for example, `3.14`). This prevents drift as new patch
...
Packaged projects created with `uv init` are ready to build with `uv_build`:
